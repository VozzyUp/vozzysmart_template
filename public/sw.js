...
const CACHE_VERSION = 'vozzysmart-v1'
  ...
  event.waitUntil(
    caches.keys()
      .then((cacheNames) => {
        return Promise.all(
          cacheNames
            .filter((name) => name.startsWith('vozzysmart-') && name !== STATIC_CACHE && name !== DYNAMIC_CACHE)
            .map((name) => {
